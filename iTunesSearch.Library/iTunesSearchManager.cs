...
public class iTunesSearchManager
{
    ...
    /// <returns></returns>
    private static async Task<T> MakeAPICall<T>(string apiCall)
    {
        HttpClient client = new();

        //  Make an async call to get the response
        var stream = await client.GetStreamAsync(apiCall);
    ...
    {
        return (T)JsonSerializer.Deserialize(stream, typeof(T), TryDeserializeContext.Default)!;
    }
